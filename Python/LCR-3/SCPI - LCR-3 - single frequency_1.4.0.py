...
# Defining channel combinations at which measurements shall be performed. In this example, we will perform
# measurements at three different channels each in 4-point configuration.
channels = {"channel_one": "1,0,1,0,1,0,1,0",   # 1. measurement channel: Main Port
            "channle_two": "3,1,3,2,3,3,3,4",   # 2. measurement channel (internal multiplexer): int-1 int-2 int-3 int-4
            "channel_three": "3,5,3,6,3,7,3,8"} # 3. measurement channel (internal multiplexer): int-5 int-6 int-7 int-8
...
with serial.Serial(port=COM_PORT, timeout=TIMEOUT, write_timeout=TIMEOUT,) as serial_connection:
    ...
    # Using, e.g., the resistance model for a measurement, i.e., model 5
    model_number = 5
    MODEL_TYPE_COMMAND = "FUNCTION:MEASUREMENT:TYPE " + str(model_number) + "\n"
    serial_connection.write(MODEL_TYPE_COMMAND.encode())
    print("Acknowledgement of the model type command: " + serial_connection.readline().decode().strip())
    MODEL_TYPE_CHECK_COMMAND = "FUNCTION:MEASUREMENT:TYPE?\n"
    serial_connection.write(MODEL_TYPE_CHECK_COMMAND.encode())
    print("Model type: " + serial_connection.readline().decode().strip())
    print("Acknowledgement of the model type request: " + serial_connection.readline().decode().strip())
    print("\n")
    ...
    FREQUENCY_CHECK_COMMAND = "FREQUENCY? 1\n"
    ...
    # Generating empty ".csv"-files that will be filled with the measurement data
    for channel in channels:
        data_file_name = channels[channel] + ", " + str(frequency) + " Hz.csv"
    ...
    # Defining how many measurements shall be performed, in this example 5.
    number_of_measurements = 5

    # Iterating over the numer of adjusted measurements
    for i in range(number_of_measurements):
        print(str(i+1) + ". measurement run")
        # Iterating over the configured channels
        for channel in channels:
            # Resetting the port configuration stack to prevent unwanted ports to be configured. This is important for
            # the next command
            MEASUREMENT_CHANNEL_RESET_COMMAND = "FUNCTION:MEASUREMENT:CHANNEL:RST\n"
            serial_connection.write(MEASUREMENT_CHANNEL_RESET_COMMAND.encode())
            print(
                "Acknowledgement of the measurement channel reset command: " + serial_connection.readline().decode().strip())

            # Setting the measurement configuration to a measurement as specified in the dictionary at the top of this
            # script
            MEASUREMENT_CHANNEL_COMMAND = "FUNCTION:MEASUREMENT:CHANNEL:ADD " + channels[channel] + "\n"
            serial_connection.write(MEASUREMENT_CHANNEL_COMMAND.encode())
            print("Acknowledgement of the measurement channel command: " + serial_connection.readline().decode().strip())
            MEASUREMENT_CHANNEL_CHECK_COMMAND = "FUNCTION:MEASUREMENT:CHANNEL:ADD?\n"
            serial_connection.write(MEASUREMENT_CHANNEL_CHECK_COMMAND.encode())
            ...
            number_of_runs = 1           # Total number of measurement runs
            ...
            while run_counter < number_of_runs:
                ...
                # In case of unfortunate measurement settings, additionally to the measurement data, the device sends
                # the information whether an over-current or over-voltage event have been detected. In this example,
                # this information is not stored in the measurement data frame.
